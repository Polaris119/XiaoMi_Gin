...
func (con GoodsCateController) Add(c *gin.Context) {
	// 获取顶级分类
	goodsGateList := []models.GoodsCate{}
	models.DB.Where("pid = 0").Find(&goodsGateList)

	c.HTML(http.StatusOK, "admin/goodsCate/add.html", gin.H{
		"goodsCateList": goodsGateList,
	})
}
...
func (con GoodsCateController) Edit(c *gin.Context) {
	...
	if err != nil {
		con.Error(c, "数据有误", "/admin/goodsCate")
		return
	}
...
func (con GoodsCateController) DoEdit(c *gin.Context) {
	...
	if err3 != nil {
		con.Error(c, "请输入正确的排序值", "/goodsCate/add")
		return
	}
